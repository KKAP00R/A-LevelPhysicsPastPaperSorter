...
def browsefn():
    ...
    Label(window2, text = "ENTER 1 for June session, 2 for November and 3 for March").grid(row = 3)
    global entry1
    global entry3
    global entry2
...
def savedata11():
    ...
    page10 = p10.get()
    page11 = p11.get()
    path = "C:/Users/Karan Kapoor/OneDrive/Desktop/PROGRAMS/5"
    name = os.path.join(path, s1 + ".txt")
    f = open(name, "w+")
    f.write("1.  " + string1 + "\n")
    f.write(" 1P. " + page1 + "\n")
    f.write("2.  " + string2 + "\n")
...
def choice():
    ...
    Label(window5, text = "IF ENTERING 2 TOPICS FOR SEARCH, DOUBLE SPACE BETWEEN THEM").grid(row = 2)
    global input1
    global input2
    input1 = Entry(window5)
    input1.grid(row = 0, column = 1)
    next1 = Button(window5, text = "SEARCH", width = 20, command = choiceprocess1)
    next1.grid(row = 1, column = 1)
    window5.mainloop()
def choiceprocess1():
    global choice1
    global choice2
...
def presortfunction1():
    path_file2 = "C:/Users/Karan Kapoor/OneDrive/Desktop/PROGRAMS/5"
    global arr
    global z1
    global reqcon
    global recqon
...
def pdfextraction():
    ...
    pdfmerger2()
def pdfmerger2():
    path_thisfile = "C:/Users/Karan Kapoor/OneDrive/Desktop/PROGRAMS/ALEVEL"
    output_file = 'TOPIC' + str(choice1) + " QUESTIONS LOADED.pdf"
    output_total = os.path.join(path_thisfile, output_file)
    print(finalarr)
    pdfMerger = PyPDF2.PdfFileMerger()
    for pdf in finalarr: 
        pdfMerger.append(pdf) 
    with open(output_total, 'wb') as f: 
        pdfMerger.write(f)
        pdfMerger.close()
def fileremover():
    for s in finalarr:
